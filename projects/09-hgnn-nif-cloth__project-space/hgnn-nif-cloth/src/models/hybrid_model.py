...
def load_model_from_config(config_path: str, device: torch.device) -> 'HGNN_NIF_ClothModel':
    """
    Load HGNN_NIF_ClothModel from a YAML configuration file.
    ...
    Returns:
        Configured HGNN_NIF_ClothModel instance

    Example:
        >>> model = load_model_from_config('configs/rtx3090_large.yaml', torch.device('cuda'))
    """
    import yaml
    ...
    model = HGNN_NIF_ClothModel(
        node_feat_dim=model_cfg.get('node_feat_dim', 3),
        latent_dim=model_cfg.get('latent_dim', 64),
        hidden_dim=model_cfg.get('hidden_dim', 64),
        hgnn_layers=model_cfg.get('num_gnn_layers', 2),
        siren_layers=model_cfg.get('siren_layers', 3),
        siren_hidden_dim=model_cfg.get('siren_hidden_dim', 128),
        num_heads=model_cfg.get('num_attention_heads', 4),
        energy_threshold=model_cfg.get('energy_threshold', 0.1),
